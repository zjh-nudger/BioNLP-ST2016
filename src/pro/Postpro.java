		...
		for (int i = 0; i < sampleFiless.length; i++) {
			...
			for (int j = 0; j < oneFileList.size(); j++) {
				...
				if (!ec.getClassName(Integer.valueOf(predicList.get(count))).startsWith("Not")) {
					...
					if (element1.contains("Bacteria")) {
						element2 = "Location" + ":" + t[1].split("_")[0]; 
						oneOutList.add("R" + ++sum + "\t" + type + " " + element1 + " " + element2);
					}
					else {
						element1 = "Location" + ":" + t[0].split("_")[0];
						oneOutList.add("R" + ++sum + "\t" + type + " " + element2 + " " + element1);
					}
					all++;
				}
				count++;
			}
			WFileWriter.writeArrayListToFile(oneOutList, new File(sampleFiless[i].getAbsolutePath().replace("\\a2_2\\", "\\a2_predict\\").replace("gt", "a2")));
		}
