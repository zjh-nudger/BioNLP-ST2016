...
public class WFileReader {
	...
	public static ArrayList<String> getFileLine_BB3(File source){		
		...
		try {
			...
			String singLine = "";
			while((singLine = br2.readLine()) != null){			
				if (!singLine.split("\t")[1].startsWith("Paragraph") | !singLine.split("\t")[1].startsWith("Title")) {
					result.add(singLine);
				}
			}
		} catch (IOException e) {
	...
	public static ArrayList<String> getFileLine_BB3_a2(File source){		
		ArrayList<String> result = new ArrayList<String>();
		try {
			br2 = new BufferedReader(new FileReader(source));		
			...
			String singLine = "";
			while((singLine = br2.readLine()) != null){			
				if(singLine.startsWith("R")) result.add(singLine);
			}
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
